...
bool Join::execute(Server *server, std::string args, int clientFd) {
	if (args.empty() || clientFd < 0) {
		std::string replyError =
				":" + server->getServerName() + " 461 " + server->clients[clientFd].getNickName() + " JOIN " +
				" :No such channel \r\n";
	...
	while (std::getline(iss, channelName, ',')) {
		...
		if (channel == nullptr) {
			...
			std::string operatorMessage =
					":" + server->getServerName() + " 381 " + server->clients[clientFd].getNickName() +
					" :You are now an IRC operator \r\n";
			send(clientFd, operatorMessage.c_str(), operatorMessage.size(), 0);

			std::cout << "Channel " << channelName << " created and client " << server->clients[clientFd].getNickName()
					  << " added to it." << std::endl;
		} else {
			if (channel->getInviteOnly() && !channel->isUserInvited(client)) {
				// Channel is invite-only and user is not invited
				std::string replyError =
						":" + server->getServerName() + " 473 " + server->clients[clientFd].getNickName() + " " +
						channelName + " :Cannot join channel (+i) - not invited \r\n";
				send(clientFd, replyError.c_str(), replyError.size(), 0);
				return false;
			}
			if (channel->hasUser(client)) {
				...
				std::string replyError =
						":" + server->getServerName() + " 443 " + server->clients[clientFd].getNickName() + " " +
			...
			if (channel->getHasKey()) {
				...
				if (providedKey != channel->getKey()) {
					...
					std::string replyError =
							":" + server->getServerName() + " 475 " + server->clients[clientFd].getNickName() + " " +
			...
			if (channel->getUserLimitValue() == channel->getUsers().size() && channel->getHasLimit()) {
				// Channel is full
				std::string replyError =
						":" + server->getServerName() + " 471 " + server->clients[clientFd].getNickName() + " " +
						channelName + " :Cannot join channel (+l) - channel is full \r\n";
				send(clientFd, replyError.c_str(), replyError.size(), 0);
				return false;
			}
